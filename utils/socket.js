...
function socket(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('joined-user', (data) => {
            ...
            io.to(data.roomname).emit('joined-user', { username: data.name });
        ...
        socket.on('setLocal', (slug,data) => {
            console.log(slug)
            io.to(roomname).emit(data.roomname).emit('setLocal', slug,data)
        })

        socket.on('getLocal', (slug) => {
            console.log(slug)
            io.to(roomname).emit(data.roomname).emit('getLocal', data.username)
        })
